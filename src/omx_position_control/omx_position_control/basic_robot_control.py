...
Sends joint-space motion commands to the OpenManipulator-X using the `goal_joint_space_path` service. 
Waits for the service to become available, commands the robot to move to predefined joint configurations, and verifies 
whether each motion was successfully planned and executed.
...
class BasicRobotControl(Node):
    ...
    def send_pose_request(self, joint_positions, path_time):
        ...
        self.get_logger().info(
            f"Sending request to move to position: {joint_positions} in {path_time} seconds."
        )
            ...
            if self.future.done():
                try:
                    ...
                    if response.is_planned:
                        ...
                        self.get_logger().error(
                            f"Movement failed! Result: {response.result}"
                        )
...
def main(args=None):
    ...
    robot_control_node = BasicRobotControl()
    ...
    pose2 = [0.0, -1.50, 1.23, 0.33, 0.0]  # Second pose

    # 1. Move to the first pose.
    robot_control_node.send_pose_request(pose1, 2.0)
    robot_control_node.wait_for_response()

    robot_control_node.get_logger().info(
        "Waiting for 2 seconds before moving to the next pose..."
    )
    time.sleep(2)

    # 2. Move to the second pose.
    robot_control_node.send_pose_request(pose2, 2.0)
    robot_control_node.wait_for_response()
