...
Implements an Inverse Kinematics (IK) service for a robotic manipulator. 
Given a desired Cartesian pose (position and orientation), computes the corresponding joint angles using a
geometric IK approach and enforces symmetric joint limits to ensure safe and feasible solutions.
...
class InverseKinematicsNode(Node):
    ...
    # Takes a Cartesian pose (x, y, z, quaternion), returns the corresponding joint angles, and enforces joint limits for safety.
    ...
    def __init__(self):
        ...
        # Joint 2 (Shoulder)
        self.joint2_min = np.deg2rad(-120)
        self.joint2_max = np.deg2rad(120)
        
        # Joint 3 (Elbow)
        self.joint3_min = np.deg2rad(-120)
        self.joint3_max = np.deg2rad(120)

        # Joint 4 (Wrist)
        self.joint4_min = np.deg2rad(-120)
        self.joint4_max = np.deg2rad(120)
